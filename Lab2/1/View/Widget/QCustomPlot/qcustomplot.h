...
void QCPDataContainer<DataType>::add(const DataType& data) {
    if (isEmpty() ||
        !qcpLessThanSortKey<DataType>(
            data,
            *(constEnd() - 1)))  // quickly handle appends if new data key is
                                 // greater or equal to existing ones
    {
        mData.append(data);
    } else if (qcpLessThanSortKey<DataType>(
                   data,
                   *constBegin()))  // quickly handle prepends using
                                    // preallocated space
    {
...
void QCPDataContainer<DataType>::removeBefore(double sortKey) {
    ...
    mPreallocSize +=
        itEnd -
        it;  // don't actually delete, just add it to the preallocated block (if
...
void QCPDataContainer<DataType>::remove(double sortKeyFrom, double sortKeyTo) {
    ...
    QCPDataContainer<DataType>::iterator it =
        std::lower_bound(begin(),
                         ...
                         DataType::fromSortKey(sortKeyFrom),
                         qcpLessThanSortKey<DataType>);
    QCPDataContainer<DataType>::iterator itEnd =
        std::upper_bound(it,
                         end(),
                         DataType::fromSortKey(sortKeyTo),
                         qcpLessThanSortKey<DataType>);
...
double QCPAbstractPlottable1D<DataType>::selectTest(const QPointF& pos,
    ...
    if (minDistIndex != mDataContainer->size())
        selectionResult.addDataRange(
            QCPDataRange(minDistIndex, minDistIndex + 1),
            false);
